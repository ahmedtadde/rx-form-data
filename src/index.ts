...
export default function RxFormData(
  ...
  subscribe: (subscriber: FormFieldSubscriber) => () => void;
  dispatch: ProgramInterfaceActionFn;
}> {
  ...
  const events = eventsInitialization($target, handler, repository);
  ...
  return Object.freeze({
    ...
    subscribe: (subscriber: FormFieldSubscriber) => {
    ...
    dispatch: (type: ProgramInterfaceActionType, payload?: unknown): void => {
      switch (type) {
